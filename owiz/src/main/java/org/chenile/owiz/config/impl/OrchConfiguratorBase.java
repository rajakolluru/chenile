...
public class OrchConfiguratorBase<InputType> implements OrchConfigurator<InputType>{
	...
	private void processChain(Map<String,CommandDescriptor<InputType>> commandCatalog) throws OwizConfigException {
	...
	protected Command<InputType> makeCommand(CommandDescriptor<InputType> cd) throws OwizConfigException {
		if (cd.getComponentName() == null) {
			throw new OwizConfigException("Command " + cd.getId() + " has no component name");
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected Command<InputType> lookupCommand(String componentName) throws OwizConfigException {
		Object obj = null;
		if (beanFactoryAdapter !=  null )
			obj = beanFactoryAdapter.lookup(componentName);
		if (obj == null) {
			try {
				obj = Class.forName(componentName).getDeclaredConstructor().newInstance();
			}catch(Exception e) {
				throw new OwizConfigException("Error in instantiating class " + componentName,e);
			}
		}

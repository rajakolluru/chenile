...
public abstract class AbstractSearchServiceImpl implements SearchService<Map<String, Object>>{
	...
	public final SearchResponse search(SearchRequest<Map<String, Object>> searchRequest) {
		...
		if (queryMetadata == null) 
			throw new NotFoundException(ErrorCodes.QUERY_ID_NOT_FOUND.ordinal(),"Query " + searchRequest.getQueryName() + " not found");
		
		if (searchRequest.getCannedReportName() != null) {
			CannedReport cannedReport = cannedReportStore.retrieve(searchRequest.getCannedReportName(),
					searchRequest.getQueryName());
			if (cannedReport == null && searchRequest.isSaveChangesToCannedReport()) {
				saveCannedReport(searchRequest, null);
			}else if (cannedReport != null) {
				/**
				 * Merging search request causes the same sort criteria to be added multiple times.
				 * It is expected the user has selected the appropriate filters/sort criteria/hidden columns to be persisted for the report.
				 */
//				mergeSearchRequests(searchRequest,(SearchRequest<Map<String,Object>>)cannedReport.getSearchRequest());
				saveCannedReport(searchRequest, cannedReport.getId());
			}
		}		
		...
		if (queryMetadata.getAcls() != null && queryMetadata.getAcls().length > 0) {
			// if (!securityService.isAllowed(contextContainer.getUser(), queryMetadata.getAcls()))
				// throw new RuntimeException("User " + contextContainer.getUser() + " not authorized to execute query " + searchRequest.getQueryName());
		}
	...
	private void saveCannedReport(SearchRequest<Map<String, Object>> searchRequest, String documentId) {
		CannedReport cannedReport = new CannedReport();
		cannedReport.setQueryName(searchRequest.getQueryName());
		cannedReport.setCannedReportName(searchRequest.getCannedReportName());
		cannedReport.setSearchRequest(searchRequest);
		cannedReport.setApplicableToAll(searchRequest.isPublishCannedReportToEveryone());
		// cannedReport.setTenantId(contextContainer.getRegion());
		if(searchRequest.isPublishCannedReportToEveryone())
			cannedReport.setUserId(contextContainer.getEmployeeId());
		
		cannedReport.setId(documentId);
		
		cannedReportStore.store(cannedReport);
	}
	...
	protected void enhanceFiltersUsingMetadata(EnhancedSearchRequest searchInput,QueryMetadata queryMetadata ) {
		...
		if (filters == null || cmdmap == null) return;	
		for (String name: filters.keySet()) {				
			ColumnMetadata cmd = cmdmap.get(name);			
			if (cmd == null || !cmd.isFilterable()) { // ignore stuff in filters which is not filterable
				System.err.println("Warning: Filter name " + name + " is not filterable but has been passed as a filter!");
				continue;
			}
	...
	protected void constructBetweenQuery(Map<String, Object> enhancedFilters, String name, Object value, ColumnMetadata columnMetadata) {
		if (null == columnMetadata) return;
		if (!(value instanceof List)) {
			throw new ServerException("Value: " + value + " should be an array for the range operation to be executed.");
		};
		
		ColumnType columnType = columnMetadata.getColumnType();
		
		// No between operation for Checkbox and DropDown Type
		if (columnType == ColumnType.CheckBox || columnType == ColumnType.DropDown) return;
		...
		int size = list.size();
		if (1 == size) {
			// If list contains only one element then add the same element again so that the between query contains two elements.
			list.add(list.get(0));
		}
		...
		if ("".equals(firstStr) && "".equals(secondStr)) return;
	...
	protected void buildOrderByClause(Map<String,Object> filters,List<SortCriterion> sortCriteria,QueryMetadata queryMetadata) {		
		if (!queryMetadata.isSortable()) return;
		...
		for (SortCriterion s : sortCriteria) {
			...
			if (StringUtils.isNotEmpty(s.getName())) {
				orberyByStringBuilder
						.append(s.getName())
						.append((s.isAscendingOrder() ? " ASC " : " DESC "));
			} else {
				orberyByStringBuilder.append(s.getIndex()).append(
					(s.isAscendingOrder() ? " ASC " : " DESC "));
			}
	...
	protected abstract SearchResponse doSearch(EnhancedSearchRequest searchInput, SearchResponse searchResponse, QueryMetadata queryMetadata); 
	...
	protected SearchResponse makeSearchResponse(QueryMetadata queryMetadata,EnhancedSearchRequest searchInput) {
		...
		List<CannedReport> cannedReports = cannedReportStore.getAllCannedReportsForUserTenant(searchInput.originalSearchRequest.getQueryName());
		searchResponse.setAvailableCannedReports(cannedReports);
		return searchResponse;
	}
	...
	/**
	 * Merge the second search request to the first search request. The first search request will be enhanced
	 * @param one
	 * @param two
	 */
	...
	private void mergeSearchRequests(SearchRequest<Map<String,Object>> one, SearchRequest<Map<String,Object>> two) {
		...
		/**
		 * Merging NumRowsInPage not required, because the default value has been set at the model level.
		 * If we merge, then the value from the elastic search always overrides the actual SearchRequest.
		 */
